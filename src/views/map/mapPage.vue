        ...
        <div  :style="{left:item.lng+'px',top:100+item.lat +'px',position:'absolute',cursor:'pointer'}" v-drag="false"
            ...
            <div class="contentBox"  :id="'contentDivA'+index">
                ...
                <div class="iframeBox" v-else>
                    ...
                    <iframe v-else :src="'http://'+$global.deviceUrl+'/embedded.html'" ref="iframe" :id="'iframe'+item.id" width="100%"
                            height="100%"></iframe>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async getMapInfo(){
          ...
          const lng = this.points[0].lng - 0.0013;
          const lat = this.points[0].lat;
          console.log(this.points,'pointssssssss');
          this.center.lng = Number(lng.toFixed(2))
